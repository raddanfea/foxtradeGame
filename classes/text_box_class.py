...
class TextBoxObject:
    def __init__(self, game, squish_offset):
        self.squish_offset = game.screen.screen.get_width() * squish_offset
        self.offset = int(game.screen.screen.get_width() * 0.01)
        self.x = game.screen.screen.get_width()
        self.y = game.screen.screen.get_height() * 0.6
        self.w = game.screen.screen.get_width() - self.squish_offset * 2
        self.h = game.screen.screen.get_height() * 0.4
        self.text = ""
        self.text_state = 0
        self.font_color = (0, 0, 0)
        self.font_size = int(game.screen.screen.get_width() // 13.5*0.5)
        self.font = game.fonts.large
        self.rect = pygame.Rect((self.x, self.y), (self.w, self.h))
        self.bg_img = None

    def change_size(self, game, x, img):
        self.x = game.screen.screen.get_width() * x
        self.y = game.screen.screen.get_height() * 0.6
        self.w = game.screen.screen.get_width() * (1-x) - self.squish_offset * 2
        self.h = game.screen.screen.get_height() * 0.4
        self.rect = pygame.Rect((self.x, self.y), (self.w, self.h))
        self.bg_img = game.gui_images.images[img]

    def draw_box(self, game):
        game.screen.screen.blit(self.bg_img, self.rect)
    ...
    def draw_text(self, game):
        ...
        for x, line in enumerate(text_lines):
            ...
            game.screen.screen.blit(textobj, textrect)
