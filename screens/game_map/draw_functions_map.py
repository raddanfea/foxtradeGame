...
def draw_game_map(game):
    game.screen.screen.blit(game.game_map.map_img[day_night_time_to_shader(game.time)], (0, 0),
                            (*game.player.player_offset, *game.screen.screen.get_size()))


def draw_game_map_overhead(game):
    game.screen.screen.blit(game.game_map.map_img_over[day_night_time_to_shader(game.time)], (0, 0),
                            (*game.player.player_offset, *game.screen.screen.get_size()),
...
def draw_night_shader(game):
    ...
    game.screen.screen.blit(game.night, (0, 0),
                            special_flags=BLEND_ALPHA_SDL2)


def draw_player(game):
    game.screen.screen.blit(game.player.get_blit(day_night_time_to_shader(game.time)),
                            (int(game.screen.screen.get_width() * 0.5) - 40,
                             int(game.screen.screen.get_height() * 0.5) - 40),
                            special_flags=BLEND_ALPHA_SDL2)
...
def is_in_village(game):
    start_type = imgColorToType(
        game.game_map.map_walls.get_at((int(game.player.player_offset[0] + game.screen.screen.get_width() * 0.5),
                                        int(game.player.player_offset[1] + game.screen.screen.get_height() * 0.5))))
