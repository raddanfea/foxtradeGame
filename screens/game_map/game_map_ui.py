...
def game_map_ui(game: GameObject):
    ...
    draw_text(
        text=f'{x + ox}  {y + oy} ',
        color=(0, 0, 0), surface=game.screen.screen, x=15, y=30, w=50, h=50, center=False, font=game.fonts.default)

    draw_text(
        text=f'{int(game.mainClock.get_fps())} ',
        color=(0, 0, 0), surface=game.screen.screen, x=15, y=50, w=50, h=50, center=False, font=game.fonts.default)

    game.screen.screen.blit(game.gui_images.images['map_info_bg'],
                            (int(0),
                             int(game.screen.screen.get_height() -
                                 game.gui_images.images['map_info_bg'].get_height())),
                            special_flags=BLEND_ALPHA_SDL2)

    game.screen.screen.blit(game.item_images.images['bread'],
                            (int(game.screen.screen.get_width() * 0.01),
                             int(game.screen.screen.get_height() -
    ...
    game.screen.screen.blit(game.item_images.images['coin'],
                            (int(game.screen.screen.get_width() * 0.03 + game.item_images.images['bread'].get_width() * 2),
                             int(game.screen.screen.get_height() -
                                 game.gui_images.images['map_info_bg'].get_height() * 0.4 -
                                 game.item_images.images['bread'].get_height() * 0.5)
                             ))
    ...
    draw_text(
        ...
        color=(0, 0, 0), surface=game.screen.screen,
        x=int(game.screen.screen.get_width() * 0.03 + game.item_images.images['bread'].get_width() * 3.5),
        y=int(game.screen.screen.get_height() -
              game.gui_images.images['map_info_bg'].get_height() * 0.4 -
