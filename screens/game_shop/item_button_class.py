...
class ItemObject(ChoiceButton):
    def __init__(self, game, image_name, x, y):
        ...
        self.x = x * game.window.screen.get_width()
        self.y = y * game.window.screen.get_height() + game.window.screen.get_height() * 0.1
    ...
    def draw_item_data(self, game, item_name, item_data):
        ...
        draw_text(
            ...
            x=self.x - game.window.screen.get_width()*0.11,
            y=self.y - game.window.screen.get_width()*0.015,
            w=1, h=1,
            center=False,
        ...
        draw_text(
            text=f'{item_data[-1]}',
            color=(0, 0, 0), surface=game.window.screen,
            x=self.x + game.window.screen.get_width() * 0.04,
            y=self.y - game.window.screen.get_width() * 0.015,
            w=1, h=1,
            center=False,
            font=game.fonts.large)

        game.window.screen.blit(game.item_images.images[icon_name],
                                (self.x + game.window.screen.get_width() * 0.07,
                                 self.y - game.window.screen.get_width() * 0.015,))
